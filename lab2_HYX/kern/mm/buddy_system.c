...
//可以使用一个完全二叉树来表示内存块的分配状态，其中 longest 数组保存每个节点中可用的最长空闲块的长度。
struct buddy {
    size_t size;//表示伙伴系统管理的内存大小或层级深度
    uintptr_t *longest;//longest 是一个指向 uintptr_t 类型数组的指针，用于管理和记录伙伴系统每个层级的状态。
    size_t curr_free;//前空闲内存块的数量，或者空闲页的数量。
    struct Page *begin_page;//begin_page 指向一个 Page 类型的结构体，通常代表伙伴系统管理的内存块的起始位置。
};
...
buddy_init_memmap(struct Page *base, size_t n) {
    ...
    size_t s;
    if(!IS_POWER_OF_2(n)){
        s = last_power_of_2(n);//// 如果 n 不是 2 的幂，则使用小于 n 的最大 2 的幂作为 buddy 大小
    }
    else{
        s = n;
    }
    ...
    size_t node_size = buddy->size * 2;//// 二叉树根节点的块大小为 buddy 的两倍
