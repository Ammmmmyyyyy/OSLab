...
static inline void print_pgfault(struct trapframe *tf) {//struct trapframe *tf：捕获 CPU 异常状态的结构体，包含页面故障的相关信息，例如：
    cprintf("page fault at 0x%08x: %c/%c\n", tf->badvaddr,//故障地址 (badvaddr)。K：内核模式。U：用户模式 W：写入引发的页面故障（CAUSE_STORE_PAGE_FAULT）R：读取引发的页面故障。
            trap_in_kernel(tf) ? 'K' : 'U',
            tf->cause == CAUSE_STORE_PAGE_FAULT ? 'W' : 'R');//cause：页面故障的原因。
}
