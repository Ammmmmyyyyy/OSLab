...
pte_t *get_pte(pde_t *pgdir, uintptr_t la, bool create) {//uintptr_t la:要查找的虚拟地址。是否允许创建缺失的页表：true: 如果页表不存在，会分配新的页面用作页表。false: 如果页表不存在，直接返回 NULL。
    /*
     *
     ...
     */
    pde_t *pdep1 = &pgdir[PDX1(la)];//从虚拟地址 la 的高 9 位提取一级页目录索引。
    if (!(*pdep1 & PTE_V)) {
        struct Page *page;
    ...
    /*
    PDX0(la):
从虚拟地址 la 的中间 9 位提取二级页目录索引。
((pde_t *)KADDR(PDE_ADDR(*pdep1)))[PDX0(la)]:
获取虚拟地址 la 对应的二级页目录项
    */
    pde_t *pdep0 = &((pde_t *)KADDR(PDE_ADDR(*pdep1)))[PDX0(la)];//ADDR(PDE_ADDR(*pdep1)):将一级页目录项中存储的物理地址转换为内核虚拟地址。
//    pde_t *pdep0 = &((pde_t *)(PDE_ADDR(*pdep1)))[PDX0(la)];
    if (!(*pdep0 & PTE_V)) {
    	...
    	set_page_ref(page, 1);
    	uintptr_t pa = page2pa(page);//这段代码将物理页面结构体（struct Page）转换为该页面对应的物理地址（pa）。
    	memset(KADDR(pa), 0, PGSIZE);
 //   	memset(pa, 0, PGSIZE);
    ...
    /*
 KADDR(PDE_ADDR(*pdep0)):
将二级页目录项中存储的物理地址转换为内核虚拟地址。
PTX(la):
从虚拟地址 la 的低 9 位提取页表索引。
((pte_t *)KADDR(PDE_ADDR(*pdep0)))[PTX(la)]:
获取虚拟地址 la 对应的页表项。
 */
...
void page_remove(pde_t *pgdir, uintptr_t la) {
    ...
    if (ptep != NULL) {
        page_remove_pte(pgdir, la, ptep);////删除这个页表项的映射.pgdir是页表基址(satp)
    }
}
...
int page_insert(pde_t *pgdir, struct Page *page, uintptr_t la, uint32_t perm) { //将一个物理页面（struct Page）映射到指定的虚拟地址（la）并更新相关的页表项。la: 要映射的虚拟地址。
    pte_t *ptep = get_pte(pgdir, la, 1);
    ////先找到对应页表项的位置，如果原先不存在，get_pte()会分配页表项的内存
    if (ptep == NULL) {
        return -E_NO_MEM;
    }
    page_ref_inc(page);//增加物理页面的引用计数，表示该页面被一个新的虚拟地址引用。
    if (*ptep & PTE_V) {//PTE_V 标志：表示页表项有效。
        struct Page *p = pte2page(*ptep);
        if (p == page) {
            page_ref_dec(page);//果当前映射的物理页面与目标页面相同（p == page），减少刚刚增加的引用计数。
            //如果已经存在相同的映射（p == page），实际上没有新增的引用，page_ref_inc 多增加了一次
        } else {
            page_remove_pte(pgdir, la, ptep);//当前映射的物理页面与目标页面不同：调用 page_remove_pte 移除旧的映射，释放旧页面
        }
    }
    //page2ppn(page): 获取页面的物理页号（PPN）。PTE_V | perm: 设置有效位和权限位
...
static void check_pgdir(void) {
    ...
    assert(page_insert(boot_pgdir, p1, 0x0, 0) == 0);//把这个物理页面通过多级页表映射到0x0   page_insert:将物理页面 p1 映射到虚拟地址 0x0。
    pte_t *ptep;
    assert((ptep = get_pte(boot_pgdir, 0x0, 0)) != NULL);//获取虚拟地址 0x0 对应的页表项指针。
    assert(pte2page(*ptep) == p1);//验证页表项存储的物理页面与 p1 相同
    assert(page_ref(p1) == 1);//page_ref:检查物理页面 p1 的引用计数是否为 1

   //KADDR 转换页目录和页表的物理地址为内核虚拟地址。
    ptep = (pte_t *)KADDR(PDE_ADDR(boot_pgdir[0]));
    ptep = (pte_t *)KADDR(PDE_ADDR(ptep[0])) + 1;
    assert(get_pte(boot_pgdir, PGSIZE, 0) == ptep);//验证页表项的分配和层级结构是否正确

    p2 = alloc_page();
    assert(page_insert(boot_pgdir, p2, PGSIZE, PTE_U | PTE_W) == 0);//将物理页面 p2 映射到虚拟地址 PGSIZE，并设置用户和写权限。
    ...
    assert(page_insert(boot_pgdir, p1, PGSIZE, 0) == 0);//将 p1 映射到虚拟地址 PGSIZE，替换掉原来的 p2。
    ...
    //pd0 指向 二级页目录，它是由一级页目录的第一个条目（boot_pgdir[0]）指向的。
    //pd1 指向 一级页目录，即 boot_pgdir。
    //page2kva:将物理页面转换为内核虚拟地址，以便直接访问二级页目录。
...
void *kmalloc(size_t n) {////kmalloc 是一个内核内存分配函数,分配至少n个连续的字节，这里实现得不精细，占用的只能是整数个页。
    void *ptr = NULL;
    struct Page *base = NULL;
    assert(n > 0 && n < 1024 * 0124);//确保分配的字节数 n 是正数且小于 1 MB  数字 0124 是八进制表示 这实际上是 约 84 KB，而不是 1 MB。
    int num_pages = (n + PGSIZE - 1) / PGSIZE;//计算需要的页面数。
    base = alloc_pages(num_pages);
    assert(base != NULL);
    ptr = page2kva(base);//将 base 指向的物理页面转换为内核虚拟地址。
    return ptr;
}
